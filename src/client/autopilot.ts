...
export function buildAutopilotSupervisorPrompt(mode: ActiveAutopilotMode): string {
  ...
  const proactive = [
    ...
    `Before sending another proactive implementation prompt, review the recent Autopilot-sent user prompts and assistant reports in the conversation. If Autopilot has already sent several similar "find the next bottleneck/optimization" prompts for the same area and the remaining work appears to be speculative micro-optimization, defensive lifetime tuning, or overdesign without fresh profiling, bug reports, failing tests, or roadmap evidence, choose "low" with a reason that human input or measurement is needed first.`,
    `Do not let the phrase "next highest-priority" alone justify another implementation when the conversation shows diminishing returns from repeated small safe changes. Proactive mode should stop before overdeveloping a subsystem.`,
