...
Before creating: check whether the extension already exists (`ls .openzerocode/tools .openzerocode/hooks .openzerocode/skills`). Prefer improving an existing one over adding a near-duplicate.
...
Need to redo a multi-agent run  → workflow (.openzerocode/workflows/*.js)
Need to change the UI           → TUI plugin (.openzerocode/tui/*.tsx)
```
...
Write to `.openzerocode/tools/<name>.ts`:

```ts
import { Effect, Schema } from "effect"
import { Def, Result } from "../../src/tool/types"
...
Write to `.openzerocode/hooks/<name>.ts` — export a Hooks object:
...
Write to `.openzerocode/skills/<name>/SKILL.md`:
...
|------|------|-----------|
| Tools | `.openzerocode/tools/*.ts` | next turn |
| Hooks | `.openzerocode/hooks/*.ts` | next turn |
| Skills | `.openzerocode/skills/*/SKILL.md` | next turn |
| Workflows | `.openzerocode/workflows/*.js` | on invoke |
| TUI | `.openzerocode/tui/*.tsx` | restart |
